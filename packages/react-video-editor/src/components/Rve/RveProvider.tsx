...
import RveData, { type IRveData } from '@store/RveData';
import { type IVideoFile, VideoFile } from '@store/VideoFile';
import PosterFile, { type IPosterFile } from '@store/PosterFile';
import MarkerFile, { type IMarkerFile } from '@store/MarkerFile';
import RveFile, { type IRveFile } from '@store/RveFile';
...
type AddPromiseFunc = { promise: (file: RveFile) => Promise<RveFile>, array: RveFile[], file: RveFile };
 ...
 * </RveProvider>
 */
export const RveProvider: React.FC<RveProviderProps> = (
  props: RveProviderProps
) => {
  const [editorData, setEditorData] = useState<RveContextType | null>(
    new RveContextType()
  );
  const { createScreenshot, loaded } = FfmpegService();

  if (loaded && !editorData?.loaded) {
    setEditorData({ visible: null, ...editorData, loaded: true });
  }
  ...
  const updateSpecificType = (file: IRveFile): RveContextType | null => {
    ...
    if (file instanceof VideoFile) {
      ...
      file instanceof PosterFile &&
      editorData?.poster?.id === file.id
    ) {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      if (primaryType === 'video') {
        ...
        const poster = new PosterFile({
          src, ...file, id: newFileId(), visible: false,
        });
  ...
  async function addFile(file: RveFile): Promise<RveFile> {
    if (file instanceof VideoFile && file.file) {
      const video = file;
      ...
      if (file.name && file.buffer) {
        video.poster = await createScreenshot(file.name, file.buffer, timestamp);
      }
    }
    return file;
  }
  ...
  const addFiles = async (files: File[]): Promise<void> => {
    ...
    const results = await Promise.allSettled(promiseArray.map((prom) => prom?.promise(prom.file)));
    ...
    newData.originals = editorData.originals ? newData.originals?.concat(editorData.originals) : editorData.originals;
  ...
  return (<RveContext.Provider value={outVal}>
      {props.children}
    </RveContext.Provider>);
};
