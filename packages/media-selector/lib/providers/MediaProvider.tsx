 ...
 * </MediaProvider>
 */
export const MediaProvider: React.FC<MediaProviderProps> = (props: MediaProviderProps) => {
  ...
  async function initFiles(files: Array<File>, updateFile: (file: IMediaFile) => void, posterExists: boolean): Promise<Array<MediaFile>> {
   ...
   */
  const addFiles = async (files: File[] | FileWithPath[] | FileList | ChangeEvent<HTMLInputElement>): Promise<void> => {
    if (!editorData) {
      return;
    }
    if (files as File[] || files as FileList) {
      files = await fromEvent(files);
    }
