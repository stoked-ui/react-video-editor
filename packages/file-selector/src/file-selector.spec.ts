...
import {FromEventInput, fromEvent, FileSystemFileHandle} from './file-selector';
...
it('should return an empty array if the passed arg is not what we expect', async () => {
    const files = await fromEvent({} as FromEventInput);
...
it('should return files if the arg is a list of FileSystemFileHandle', async () => {
    const name = 'test.json';
    const [mockFile, mockHandle]: [File, FileSystemFileHandle] = createFileSystemFileHandle(name, {ping: true}, {
        type: 'application/json'
    });
