...
export type FromEventInput = DragEvent | ChangeEvent<HTMLInputElement> | Event | FileSystemFileHandle[];

export async function fromEvent(input: FromEventInput): Promise<(FileWithPath | DataTransferItem)[]> {
    ...
    } else if (Array.isArray(input) && input.every(item => 'getFile' in item && typeof item.getFile === 'function')) {
        return getFsHandleFiles(input)
    }
    return [];
}
